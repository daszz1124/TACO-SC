...
def logger_setup(log_file_name=None, log_file_folder_name=None, filepath=os.path.abspath(__file__), package_files=[]):
    ...
    info_file_handler = logging.FileHandler(
        log_file_folder_name + '/' + log_file_name, mode="a")
...
class RateDistortionLoss(nn.Module):
    ...
    def __init__(self, lmbda=1e-2, epochs=50, clip_model_name="openai/clip-vit-base-patch32", jit_coefficient=0.005, lpips_coefficient=1.0):
        ...
        self.transform_for_clip = transforms.Compose([
            ...
            transforms.Normalize(mean=[0.48145466, 0.4578275, 0.40821073], std=[
                                 0.26862954, 0.26130258, 0.27577711])  # do_normalize
        ])

        # "openai/clip-vit-base-patch32" 이 가진 값을 그대로 사용
    ...
    def get_joint_image_text_loss(self, output, target, text_tokens, attention_mask):
        ...
        compressed_image_embeddings = self.image_encoder(
            compressed_image).image_embeds
        target_image_embeddings = self.image_encoder(target_image).image_embeds

        # loss 계산을 위해 정규화 시도
        compressed_image_embeddings = compressed_image_embeddings / \
            compressed_image_embeddings.norm(p=2, dim=-1, keepdim=True)
        target_image_embeddings = target_image_embeddings / \
            target_image_embeddings.norm(p=2, dim=-1, keepdim=True)

        text_embeddings = self.text_encoder(
            input_ids=text_tokens, attention_mask=attention_mask).text_embeds
        text_embeddings = text_embeddings / \
            text_embeddings.norm(p=2, dim=-1, keepdim=True)

        logit_scale = self.logit_scale.exp()
        logits_per_text = torch.matmul(
            text_embeddings, compressed_image_embeddings.t()) * logit_scale

        joint_image_text_loss = self.clip_loss(logits_per_text) + self.beta * torch.norm(
            compressed_image_embeddings - target_image_embeddings, p=2)
    ...
    def forward(self, output, target, text_tokens, attention_mask):
        ...
        if self.k_J > 0.0:
            out["joint_image_text_loss"] = self.get_joint_image_text_loss(
                output, target, text_tokens, attention_mask)
            out["loss"] += self.k_J * out["joint_image_text_loss"]
        if self.k_P > 0.0:
            self.loss_fn_alex = self.loss_fn_alex.to(target.device)
            out["perceptual_loss"] = self.loss_fn_alex(
                output["x_hat"], target).mean()
            out["loss"] += self.k_P * out["perceptual_loss"]

        return out


class MMEBRateDistortionLoss(nn.Module):
    """Custom rate distortion loss with a Lagrangian parameter."""

    def __init__(self, lmbda=1e-2, epochs=50, jit_coefficient=0.000, lpips_coefficient=1.0):
        super().__init__()
        self.mse = nn.MSELoss()
        self.lmbda = lmbda
        self.epochs = epochs
    ...
    def get_joint_image_text_loss(self, output, target, target_embeddings):
        return torch.zeros(1)

    def forward(self, output, target, target_embeddings=None):
        N, _, H, W = target.size()
        out = {}
        num_pixels = N * H * W
        ...
        if self.k_J > 0.0:
            out["joint_image_text_loss"] = self.get_joint_image_text_loss(
                output, target, target_embeddings)
            out["loss"] += self.k_J * out["joint_image_text_loss"]
        if self.k_P > 0.0:
            self.loss_fn_alex = self.loss_fn_alex.to(target.device)
            out["perceptual_loss"] = self.loss_fn_alex(
