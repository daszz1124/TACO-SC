...
def parse_args_for_inference(argv):
    ...
    parser.add_argument("--image_folder_root", type=str,
                        default='/home/iisc/zsd/project/VG2SC/datasets/MSCOCO/val2014', help="image folder path")
    parser.add_argument("--checkpoint", type=str, default='/home/iisc/zsd/project/VG2SC/TACO/checkpoint/lambda_0.004.pth.tar',
                        help="path of the pretrained checkpoint")
    parser.add_argument("--save_folder", type=str, default='./compression_mscocoval_resulsts', help="path to save the generated images"
                        )
    parser.add_argument("--image_list_json", type=str,
                        default='./materials/mscoco_30k_list.json')
    parser.add_argument("--cap_pair_json", type=str,
                        default='./materials/mscoco_val41k_img_cap_pair.json')
    parser.add_argument("--clip_model_name", type=str,
                        default='./clip/clip-vit-base-patch32')
...
def main(argv):
    ...
    with open(args.image_list_json, 'r') as f:
        image_list = json.load(f)[100:200]
    image_list.sort()

    with open(args.cap_pair_json, 'r') as f:
    ...
    for img_name in tqdm(image_list, desc=f"compress:"):
        ...
        print(
            f'Checkpoint: {params_name}, img_name: {img_name}, Caption: {final_caption}')

        torchvision.utils.save_image(
            best_image['image'], f'{save_folder}/figures/{img_name}', nrow=1)
        mean_csv['bpp'] += bpp
        mean_csv['psnr'] += psnr
