...
class EvalCollator:
    ...
    def __call__(self, examples):
        ...
        图像输入
            ...
            patch embedding
            ↓
            ViT Encoder: 输出 hidden_size=1536
            ↓
            线性降维 / 投影层: Linear(1536 → 1176)
            ↓
            送入 language model 的图像 embedding
        '''

        '''
        pixel_values 参数讲解 (Bsize,PatchSize,Patchdim)
        : Bsize: 批次大小
        : 图像 patch token 数量	
        : 每个 patch token 的向量维度
        '''

        '''
        image_grid_thw 参数讲解 (T,H,W)
        T: 图像的帧数
        H: 图像在 垂直方向 被切成 96 个 patch
        w: 图像在 水平方向 被切成 96 个 patch
        h * w = 图像的 patch 数量
        '''
