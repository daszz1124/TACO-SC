from mmeb_model import process_embedding, loading_model, model_args, process_embedding_with_model, loading_processor, process_batch_embedding_with_model
...
from datasets_MSCOCO import MSCOCOMMEB_TrainDataset
from datasets_image_cap import TESTImage_Cap_pair_dataset

from pytorch_msssim import ms_ssim as ms_ssim_func
from transformers import logging as tr_logging
tr_logging.set_verbosity_error()
import warnings
warnings.filterwarnings("ignore", category=FutureWarning, module="compressai", message="`torch.cuda.amp.autocast` is deprecated")

def train_one_epoch(
    model, criterion, train_dataloader, optimizer, aux_optimizer, epoch, clip_max_norm, logger, node_rank=0
):
    model.train()
    device = next(model.parameters()).device
    for i, (d, embedding_vector) in enumerate(train_dataloader):
        d = d.to(device)
        embedding_vector = embedding_vector.to(device)
        optimizer.zero_grad()
        aux_optimizer.zero_grad()
        out_net = model(d, embedding_vector)
        out_criterion = criterion(out_net, d, embedding_vector)
...
def test_epoch(epoch, test_dataset, loss_fn_alex, model, save_path, logger):
    ...
    for image, img_name, embedding_vector in test_dataset:
        img = image.to(device)
        embedding_vector = embedding_vector.unsqueeze(0).to(device)
        ...
        out_enc = model.compress(x_padded, embedding_vector)
        shape = out_enc["shape"]
        ...
        out = model.decompress(strings, shape, embedding_vector)
        ...
        logger.info(
            f"Test epoch {epoch + 1}, File name: {img_name}, PSNR: {psnr}, MS-SSIM: {mean_MS_SSIM_prob}, LPIPS: {lpips_score}, BPP: {bpp}")
...
def main(opts):
    ...
    logger = logger_setup(
        log_file_name='logs',
        log_file_folder_name=opts.save_path
    )
    ...
    csv_file = pd.DataFrame(None, index=['Best_PSNR', 'Best_LPIPS', 'Best_MS-SSIM',
                            'Best_bpp'], columns=['bpp', 'PSNR', 'MS-SSIM', 'LPIPS', 'epoch'])

    train_dataset = MSCOCOMMEB_TrainDataset(
        dataset_folder=opts.train_dataset_root_path,
        image_path="./materials/mscoco_train_name_list_larger_than_256.json",
        caption_path="./materials/mscoco_train_caption_list_larger_than_256.json",
        embedding_file_path="./encoder_materials/mscoco_train_qwen2vl_embeddings.pickle",
    ...
    net = TACO(taco_config, text_embedding_dim=1536)
    ...
    criterion = MMEBRateDistortionLoss(
        lmbda=opts.lmbda,
        epochs=opts.epochs,
        jit_coefficient=opts.joint_image_text_loss_coefficient,
        lpips_coefficient=opts.lpips_coefficient
    )

    if node_rank == 0:

        test_dataset = TESTImage_Cap_pair_dataset(
            image_dataset_folder='/home/iisc/zsd/project/VG2SC/TACO/kodak',
            path_caption_json='./materials/kodak_ofa.json',
            embedding_file_path='./encoder_materials/kodak_val_qwen2vl_embeddings.pickle',
        )
    ...
    if checkpoint != "None":
        ...
        checkpoint = torch.load(checkpoint, map_location=f'cuda:{device_id}')
    ...
    for epoch in range(last_epoch, opts.epochs):
        ...
        if (node_rank == 0):
            with torch.no_grad():
                ...
                net_eval = TACO(
                    taco_config, text_embedding_dim=1536)
                ...
                mean_PSNR, mean_MS_SSIM_prob, mean_LPIPS, Bit_rate = test_epoch(
                    epoch, test_dataset, loss_fn_alex, net_eval, f"{save_path}/figures/{epoch + 1}", logger)

                logger.info(
                    f'average_PSNR: {mean_PSNR:.4f}dB, average_MS-SSIM: {mean_MS_SSIM_prob:.4f}, average_LPIPS: {mean_LPIPS:.4f}, average_Bit-rate: {Bit_rate:.4f} bpp')
            ...
            if mean_PSNR > best_psnr:
                ...
                csv_file.loc['Best_PSNR', :] = [round(Bit_rate, 8), round(mean_PSNR, 8), round(
                    mean_MS_SSIM_prob, 8), round(mean_LPIPS, 8), (epoch + 1)]

                best_psnr_model_path = save_path + '/' + \
                    f'best_psnr_model_PSNR_{round(mean_PSNR, 5)}_MS_SSIM_{round(mean_MS_SSIM_prob, 5)}_BPP_{round(Bit_rate, 5)}_LPIPS_{round(mean_LPIPS, 5)}_epoch_{epoch + 1}.pth.tar'
            ...
            if mean_MS_SSIM_prob > best_ms_ssim:
                ...
                csv_file.loc['Best_MS-SSIM', :] = [round(Bit_rate, 8), round(
                    mean_PSNR, 8), round(mean_MS_SSIM_prob, 8), round(mean_LPIPS, 8), (epoch + 1)]

                best_ms_ssim_model_path = save_path + '/' + \
                    f'best_ms_ssim_model_PSNR_{round(mean_PSNR, 5)}_MS_SSIM_{round(mean_MS_SSIM_prob, 5)}_BPP_{round(Bit_rate, 5)}_LPIPS_{round(mean_LPIPS, 5)}_epoch_{epoch + 1}.pth.tar'
                save_checkpoint(epoch, state_dict, optimizer, aux_optimizer,
                                lr_scheduler, best_psnr, best_ms_ssim, best_bpp, best_lpips,
            ...
            if Bit_rate < best_bpp:
                ...
                csv_file.loc['Best_bpp', :] = [round(Bit_rate, 8), round(mean_PSNR, 8), round(
                    mean_MS_SSIM_prob, 8), round(mean_LPIPS, 8), (epoch + 1)]

                best_bpp_model_path = save_path + '/' + \
                    f'best_bpp_model_PSNR_{round(mean_PSNR, 5)}_MS_SSIM_{round(mean_MS_SSIM_prob, 5)}_BPP_{round(Bit_rate, 5)}_epoch_LPIPS_{round(mean_LPIPS, 5)}_{epoch + 1}.pth.tar'
                save_checkpoint(epoch, state_dict, optimizer, aux_optimizer,
                                lr_scheduler, best_psnr, best_ms_ssim, best_bpp, best_lpips,
            ...
            if mean_LPIPS < best_lpips:
                ...
                csv_file.loc['Best_LPIPS', :] = [round(Bit_rate, 8), round(
                    mean_PSNR, 8), round(mean_MS_SSIM_prob, 8), round(mean_LPIPS, 8), (epoch + 1)]

                best_lpips_model_path = save_path + '/' + \
                    f'best_lpips_model_PSNR_{round(mean_PSNR, 5)}_MS_SSIM_{round(mean_MS_SSIM_prob, 5)}_BPP_{round(Bit_rate, 5)}_epoch_LPIPS_{round(mean_LPIPS, 5)}_{epoch + 1}.pth.tar'
                save_checkpoint(epoch, state_dict, optimizer, aux_optimizer,
                                lr_scheduler, best_psnr, best_ms_ssim, best_bpp, best_lpips,
            ...
            recent_saved_model_path = save_path + '/' + \
                f'recent_model_PSNR_{round(mean_PSNR, 5)}_MS_SSIM_{round(mean_MS_SSIM_prob, 5)}_BPP_{round(Bit_rate, 5)}_LPIPS_{round(mean_LPIPS, 5)}_epoch_{epoch + 1}.pth.tar'
            save_checkpoint(epoch, state_dict, optimizer, aux_optimizer,
                            lr_scheduler, best_psnr, best_ms_ssim, best_bpp, best_lpips,
...
def ddp_or_single_process(argvs):
    ...
    save_path = f'./exp_1_checkpoints/exp_lambda_{opts.lmbda}_seed_{opts.seed}_batch_size_{opts.batch_size}_joint_image_text_loss_coefficient_{opts.joint_image_text_loss_coefficient}_lpips_coefficient_{opts.lpips_coefficient}'

    if os.path.exists(save_path):
        logger = logger_setup(log_file_name='logs',
                              log_file_folder_name=save_path)
    ...
    logger.info(
        f"exp name : exp_lambda_{opts.lmbda}_seed_{opts.seed}_batch_size_{opts.batch_size}_joint_image_text_loss_coefficient_{opts.joint_image_text_loss_coefficient}_lpips_coefficient_{opts.lpips_coefficient}")
